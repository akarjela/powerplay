 ...
 * who bowl, pace, accuracy, movement, variation. A "*" after the name is a
 * left-hander; about a third of a T20 top order bats left.
 */
type Row =
...
function squadOf(id: string, name: string, rows: readonly Row[]): Squad {
  ...
  rows.forEach((row, index) => {
    ...
    const [named, power, technique, aggression] = row;
    // A trailing "*" on a name is a left-hander, the way a scorecard marks one.
    const left = named.endsWith("*");
    const player = left ? named.slice(0, -1) : named;
    batters.push({ id: playerId, name: player, power, technique, aggression, ...(left ? { bats: "left" as const } : {}) });
    if (row.length === 8) {
